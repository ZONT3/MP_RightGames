...
[] execVM "external\scripts\medcore.sqf";
...
if (_arc) then 
{
	...
	player addAction ["<t color='#2ECC71'>Вколоть экспериментальный стимулятор</t>", {[] execVM "external\swgovno\stimpack.sqf"}, [], 10, false, true, "", "'JLTS_drugs_stimulant_battle' in (items player)"];
};
...
if (_rc) then 
{
  ...
  player addAction ["<t color='#42AAFF'>Использовать Модернизированный коагулянт</t>", {[] execVM "external\swgovno\redBacta1.sqf"}, [], 10, false, true, "", "'JLTS_drugs_bacta_red' in (items player)"];
};