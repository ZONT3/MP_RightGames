...
*/
params [["_object",objNull,[objNull]], ["_whitelist", [], [[]]]];
...
jna_minItemMember = [2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000];
...
if(hasInterface)then{
	...
	private _action_condition = "alive _target && {_target distance _this < 5} && {vehicle player == player}";
	if (count _whitelist > 0) then {
		_action_condition = _action_condition + (format [" && {%1 call ZONT_fnc_checkRole}", str _whitelist]);
	};
