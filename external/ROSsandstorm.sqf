...
if (isnil "_eyewearCheck") then {_eyewearCheck = true};
    ...
    While {time < _endtime} do {
        ...
        if (vehicle player != player) then {
            if (!vehisOpen) then {
                ...
                {if (typeOf _x == "#particlesource" && invehicle) then {deleteVehicle _x}} forEach (vehicle player nearObjects 10);
                inVehicle = true;
        ...
        if (inBuilding) then {
            ...
            {if (typeOf _x == "#particlesource" && inBuilding) then {deleteVehicle _x}} forEach (player nearObjects 10);
